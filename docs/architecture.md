...
Index building, streaming query output, JSON query output, and ranked source-list output are also command-level behaviors. They do not introduce background workers or persistent services. JSON output intentionally omits the startup warning so automation can parse a single valid JSON document.

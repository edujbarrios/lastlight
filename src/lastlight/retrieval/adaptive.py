...
from .strategies import BM25RetrievalStrategy, LexicalRetrievalStrategy
from .tokenizer import tokenize

CRITICAL_RISK_TERMS = frozenset(
    {
        "bleeding", "hemorrhage", "unconscious", "not breathing", "chest pain",
        "poison", "overdose", "seizure", "stroke", "hemorragia", "inconsciente",
        "no respira", "dolor pecho", "veneno", "sobredosis", "convulsion", "ictus",
    }
)

HIGH_RISK_TERMS = frozenset(
    {
        "water", "purify", "contaminated", "fire", "smoke", "gas", "generator",
        "hypothermia", "heatstroke", "agua", "purificar", "contaminada", "fuego",
        "humo", "gas", "generador", "hipotermia", "golpe calor",
    }
)
...
class AdaptiveRetrievalStrategy(RetrievalStrategy):
    ...
    def __init__(self, config: AdaptiveRetrievalConfig | None = None, profile: ResourceProfile | None = None) -> None:
    ...
    def search(self, query: SearchQuery, documents: list[KnowledgeDocument]) -> list[SearchResult]:
        ...
        strategy = {"lexical": self.lexical, "bm25": self.bm25}[decision.strategy]
    ...
    def plan(self, query: SearchQuery) -> RetrievalDecision:
        ...
        if constrained:
            return self._decision("lexical", risk, min(max(query.top_k, 1), 2), constraint_reason)
        if risk == "critical":
            return self._decision("lexical", risk, min(max(query.top_k, 1), 3), "critical-risk query: prefer the safer lexical refusal profile")
        if risk == "high" and self.config.mode != "accuracy":
            return self._decision("lexical", risk, min(max(query.top_k, 1), 3), "high-risk query in a safety-first mode")
        if self.config.mode == "accuracy":
            return self._decision("bm25", risk, max(query.top_k, 1), "accuracy mode with no active resource constraint")
